...
img_project1 = Image.open("images/Lykan.jpg")
...
if tabs =='Home':
    with st.container():
        ...
        with column_left:
            st.write("---")
            st.title("E.S.C Industrial Complex.")
            st.subheader("Hi :wave:, I am Sean Earl R. Escatron.")
            st.write("E.S.C Industry solving tomorrow's problems today")
            st.write("learn more by watching tutorial in youtube.")
            st.write("software is the magic thing whose importance only goes up.")
            st.write("click the link below and it will show you my creation.")
            st.write("[Access link](https://github.com/seanearl2003/MyWebpage/settings)")
        with column_right:
            pass
...
# --- Projects Section ---
    ...
    with st.container():
        ...
        with text_column:
            st.subheader("Lykan hypersport")
            st.write(
                """ 
                This is a hypercar with a unique engine also known as the beast car. 
                """
                """ 
                Twin turbo charge 3.7 liter flat six engine 750 horsepower. 
                """
                """
                0-60 less than 2.8 seconds with a top speed of 245 mph. 
                """
                """"""
            )
            st.markdown("[Access link](https://github.com/seanearl2003/MyWebpage/settings)")
    with st.container():
        image_column, text_column = st.columns((1,2)) # (image_column size, text_column size)
        with image_column:
            st.image(img_project2) # insert image
        with text_column:
            st.subheader("beach cool dude")
            st.write(
                """
                I am relaxing at this beach because programming is very difficult to understand and it almost crack my skull.
                """
            )
